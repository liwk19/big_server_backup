...
from fmoe_gates.gnn_gate import GCNGate, SageGate, Sage2Gate, GATGate, GAT2Gate
...
class FMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def __init__(self, num_expert=32, d_model=1024, world_size=1, mp_group=None, # (this param is deprecated)
        ...
        if gate == 'naive':
            self.gate = NaiveGate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'noisy':
            ...
            self.gate = MLPGate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'gcn':
            self.gate = GCNGate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'sage':
            self.gate = SageGate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'sage2':
            self.gate = Sage2Gate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'gat':
            self.gate = GATGate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
        elif gate == 'gat2':
            self.gate = GAT2Gate(d_model, num_expert, world_size, top_k, expert_drop=expert_drop)
    ...
    def forward(self, moe_inp, graph=None):
        ...
        if isinstance(self.gate, NaiveGate) or isinstance(self.gate, MLPGate):
            gate_top_k_idx, gate_score, gate = self.gate(moe_inp, return_all_scores=True)
        else:
            gate_top_k_idx, gate_score, gate = self.gate(moe_inp, graph, return_all_scores=True)
    ...
    def pseudo_forward(self, moe_inp, graph):
        r"""
        The FMoE module first computes gate output, and then conduct MoE forward
        according to the gate.  The score of the selected gate given by the
        expert is multiplied to the experts' output tensors as a weight.
        ...
        if isinstance(self.gate, NaiveGate) or isinstance(self.gate, MLPGate):
            ...
            gate_top_k_idx, gate_score, gate = self.gate(moe_inp, graph, return_all_scores=True)
        # 分数从高到低，gate_score形状也是[点数，4]，4个值分别为4个分数，相加为1。
        expert = copy.deepcopy(gate_top_k_idx)
